...
class BridgeBuildingEnv(gym.Env):
    def __init__(self):
        ...
        self.action_space = spaces.Box(
            low=np.array([-2, -0.5, 0, -2, -0.5, 0, -2, -0.5, 0], dtype=np.float32),
            high=np.array([2, 0.5, 1, 2, 0.5, 1, 2, 0.5, 1], dtype=np.float32),
            dtype=np.float32
        )
        ...
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(12,),  # 3 for ball pos, 3 for ball vel, 3 for each block pos
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        info = {
            ...
            'blocks_placed': self.blocks_placed.copy()
        }
